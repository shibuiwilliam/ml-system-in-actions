...
class ModelConfigurations:
    ...
    if not os.path.exists(model_filepath):
        model_filepath = "/circuit_breaker_pattern/models/default_iris_svc.onnx"
    label_filepath = os.getenv("LABEL_FILEPATH")
    if not os.path.exists(label_filepath):
        label_filepath = "/circuit_breaker_pattern/models/label.json"
    mode = os.path.basename(model_filepath)
