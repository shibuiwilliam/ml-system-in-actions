...
async def predict_test(background_tasks: BackgroundTasks):
    return await api.predict_test(background_tasks)


@router.get("/predict/test/label")
async def predict_test_label(background_tasks: BackgroundTasks):
    return await api.predict_test_label(background_tasks)
...
async def predict_label(data: Data, background_tasks: BackgroundTasks):
    return await api.predict_label(data=data, background_tasks=background_tasks)
