...
class ResNet18(nn.Module):
    def __init__(self, residual_block=ResidualBlock):
        ...
        self.layer1 = self.resnet_layer(residual_block=residual_block, out_channels=64, num_blocks=2, stride=1)
        self.layer2 = self.resnet_layer(residual_block=residual_block, out_channels=128, num_blocks=2, stride=2)
        self.layer3 = self.resnet_layer(residual_block=residual_block, out_channels=256, num_blocks=2, stride=2)
        self.layer4 = self.resnet_layer(residual_block=residual_block, out_channels=512, num_blocks=2, stride=2)
        self.linear = nn.Linear(512 * residual_block.expansion, num_classes)

    def resnet_layer(self, residual_block, out_channels, num_blocks, stride):
        strides = [stride] + [1] * (num_blocks - 1)
        layers = []
        for stride in strides:
            layers.append(residual_block(in_channels=self.in_channels, out_channels=out_channels, stride=stride))
